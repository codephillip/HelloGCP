...
public class QuoteEndpoint {
    ...
    // Make sure to add this endpoint to your web.xml file if this is
    //a web application.
    ...
    public CollectionResponse<Quote> listQuote(@Nullable @Named("cursor") String cursorString,
        ...
        Query<Quote> query = OfyService.ofy().load().type(Quote.class);
    ...
    public Quote insertQuote(Quote quote) throws ConflictException {
        ...
        //when we use put
        OfyService.ofy().save().entity(quote).now();
        return quote;
    }
    ...
    private Quote findRecord(Long id) {
        return OfyService.ofy().load().type(Quote.class).id(id).now();
